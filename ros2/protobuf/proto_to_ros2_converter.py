...
_GOOGLE_PROTOBUF_TIMESTAMP = '.google.protobuf.Timestamp'
_COMMON_RUNTIME_NS = 'rules_ros2_protobuf_common_runtime'


def _field_conversions(message, proto_source, proto_types_to_ros_pkgs):
    """Return (to_ros_lines, from_ros_lines, dep_pkgs, needs_common_runtime).

    Each entry in to_ros_lines / from_ros_lines is a C++ statement string
    (already indented with two spaces). dep_pkgs is the set of dep
    ros_package_names whose converters are called. needs_common_runtime is
    True if any google.protobuf.Timestamp field was encountered.
    """
    to_ros = []
    from_ros = []
    dep_pkgs = set()
    needs_common_runtime = False
    ...
    for field in message.field:
        ...
        if ftype == FieldDescriptorProto.TYPE_MESSAGE:
            ...
            if field.type_name == _GOOGLE_PROTOBUF_TIMESTAMP:
                needs_common_runtime = True
                ns = _COMMON_RUNTIME_NS
                if is_repeated:
                    to_ros.append(
                        f'  for (const auto& item : proto.{name}()) {{')
                    to_ros.append(
                        f'    {ns}::ToRos(item, &ros->{name}.emplace_back());')
                    to_ros.append('  }')
                    from_ros.append(f'  for (const auto& item : ros.{name}) {{')
                    from_ros.append(
                        f'    {ns}::FromRos(item, proto->add_{name}());')
                    from_ros.append('  }')
                else:
                    to_ros.append(
                        f'  {ns}::ToRos(proto.{name}(), &ros->{name});')
                    from_ros.append(
                        f'  {ns}::FromRos(ros.{name}, proto->mutable_{name}());'
                    )
                continue
    ...
    return to_ros, from_ros, dep_pkgs, needs_common_runtime
...
def _convert(descriptor_set, proto_sources, ros_package_name,
    ...
    messages = []
    dep_pkgs_all = set()
    needs_common_runtime_any = False
    ...
    for proto_source in proto_sources:
        ...
        to_ros_lines, from_ros_lines, dep_pkgs, needs_cr = _field_conversions(
            message, proto_source, proto_types_to_ros_pkgs)
        dep_pkgs_all.update(dep_pkgs)
        needs_common_runtime_any = needs_common_runtime_any or needs_cr
    ...
    source_context = {
        ...
        'needs_common_runtime': needs_common_runtime_any,
        'messages': messages,
    }
...
def main():
    ...
    parser.add_argument(
        ...
        default=[],
        metavar='PROTO_PATH',
        help='Relative path of a direct proto source to convert. ',
    )
    ...
    proto_sources = args.proto_source
    if not proto_sources:
        open(args.output_header, 'w').close()
        open(args.output_source, 'w').close()
        return
